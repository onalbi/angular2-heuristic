...
export class Heuristic {
    ...
    public static levenshtein(str1:string, str2:string) {
        ...
        // two rows
        let prevRow = new Array(str2.length + 1), curCol:number, nextCol:number, i:number, j:number, tmp:number;
    ...
    public static sift4(s1:string, s2:string, maxOffset:number, maxDistance?:number) {
        if (!s1 || !s1.length) {
            if (!s2) {
                return 0;
            }
        ...
        let lcss = 0;  // largest common subsequents
        let local_cs = 0; // local common substring
        let trans = 0;  // number of transpositions ('ab' vs 'ba')
        let offset_arr:any = [];  // offset pair array, for computing the transpositions
    ...
    public static getHeuristicDistance(s1:string, s2:string) {
        s1 = s1 || '';
        s2 = s2 || '';
        let maxDistance:number = Math.floor(Math.max(s1.length, s2.length) / 2);
        // perform a fast computation
        let approximateDistance:number = this.sift4(s1, s2, 5);
    ...
    public static distance(s1:string, s2:string) {
        s1 = s1 || '';
        s2 = s2 || '';
        let levScore = this.getHeuristicDistance(s1, s2);
        return (levScore / Math.max(s1.length, s2.length)) * 100;
    }

    public static similarity(s1:string, s2:string) {
